...
function cipher() {
  ...
  var str = getString();
  if(str === '') {
    document.getElementById('chiperText').innerHTML = error;
  }
  // var err = 'ERRORX';
  var textPhrase, separator;
  str = str.toUpperCase().replace(/\s/g, '').replace(/J/g, "I");
  if(str.length === 0) {
    //document.getElementById("chiperText").innerHTML = err.toUpperCase().replace(/\s/g, '').replace(/J/g, "I");
    }
  else {
    textPhrase = str[0];
  }
  ...
  document.getElementById("chiperText").innerHTML = enCodeStr;
...
function deCodeCipher() {
  ...
  text = text.join('');
  console.log(text);
  document.getElementById('chiperText').innerHTML = text;
}